...
# $1: instance name, $2: machine type, $3: zone, $4: firewall rule name, $5: username, $6: password, $7: port
if [[ -n $1 ]] && [[ $2 == e2-* ]] && [[ -n $3 ]] && [[ -n $4 ]]; then

  echo -e "${yellow}Creating instance ...${plain}"
  instance=$(gcloud compute instances create "$1" --machine-type "$2" --zone "$3" --metadata=startup-script="bash <(curl -Ls https://raw.githubusercontent.com/kaungkhantjc/GCPReady/main/install.sh) '$5' '$6' '$7'" --tags=http-server,https-server)
...
"
  ...
  echo -e "${red}Provide instance name, machine type, zone, firewall rule name, username, password and port.${plain}"
fi
